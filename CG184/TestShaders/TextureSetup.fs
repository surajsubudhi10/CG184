...
vec4 CalculatePointLight(Light _light, vec3 _normal, vec3 _fragPos, vec3 _viewPos)
{
    vec4 ambient = _light.ambient * material.ambient * texture(DiffuseMap, TexCoord);
    ...
    vec4 diffuse = (intensity * _light.diffuse) * (Kd * material.diffuse) * texture(DiffuseMap, TexCoord);

    // specular
    vec3 viewDir    = normalize(_viewPos - _fragPos);
    vec3 reflectDir = reflect(-lightDir, norm);
    float Ks        = max(pow(dot(viewDir, reflectDir), material.shininess), 0.0);
    vec4 specular   = (intensity * _light.specular) * (Ks * material.specular) * texture(SpecularMap, TexCoord);
